...
// Mobile-only styling tweaks
const MOBILE_OVERRIDES: Record<string, Partial<TvDef>> = {
  c: { body: "#8f5df0", panel: "#7b4ede", bezel: "#1c1230", acc2: "#f0e6ff" },
  e: { panel: "#76573b", panelWood: true },
};
      ...
      >
        <div style={{ position: "relative", width: stageW, height: stageH }}>
          ...
          {ordered.map((t, i) => (
            <Tv
              key={t.id}
              t={mobile ? { ...t, ...(MOBILE_OVERRIDES[t.id] ?? {}) } : t}
